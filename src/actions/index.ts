    ...
    sendContact: defineAction({
        ...
        handler: async (input) => {
            ...
            const { data, error } = await resend.emails.send({
                ...
                subject: `📩 ${input.subject ? input.subject.toUpperCase() : `contact to me`}`,
                html: `
                    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 20px; border-radius: 8px;">
                        <h2 style="color: #333; border-bottom: 2px solid #0070f3; padding-bottom: 10px;">Nuevo contacto</h2>
                        <p style="margin: 10px 0;"><strong>Nombre:</strong> ${input.name || "no name"}</p>
                        <p style="margin: 10px 0;"><strong>Email:</strong> <a href="mailto:${input.email}">${input.email}</a></p>
                        <p style="margin: 10px 0;"><strong>Teléfono:</strong> ${input.phone || "no email"}</p>
                        <p style="margin: 10px 0;"><strong>Asunto:</strong> ${input.subject || "no subject"}</p>
                        <div style="margin-top: 20px; padding: 15px; background-color: #f9f9f9; border-left: 4px solid #0070f3;">
                            <p style="margin: 0; white-space: pre-wrap;">${input.message || "no message"}</p>
                        </div>
